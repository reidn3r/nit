	...
	<div class="clearfix mb-1"></div>
		<table class="com-content-category__table category table table-striped table-bordered table-hover my-3">
				...
				<thead>
					<tr>
						...
						<?php if ($this->params->get('list_show_votes', 0) && $this->vote) : ?>
							<th scope="col" id="categorylist_header_votes" class="p-3">
								<?php echo HTMLHelper::_('grid.sort', 'COM_CONTENT_VOTES', 'rating_count', $listDirn, $listOrder); ?>
							</th>
						<?php endif; ?>
						<?php if ($this->params->get('list_show_ratings', 0) && $this->vote) : ?>
							<th scope="col" id="categorylist_header_ratings" class="p-3">
			...
			<?php foreach ($this->items as $i => $article) : ?>
				...
				<?php if (JVERSION < 4) {
					$link = Route::_(ContentHelperRoute::getArticleRoute($article->slug, $article->catid, $article->language)); 
					$state = ($this->items[$i]->state == 0);
				} else {
					$link = Route::_(RouteHelper::getArticleRoute($article->slug, $article->catid, $article->language));
					$state = ($this->items[$i]->state == ContentComponent::CONDITION_UNPUBLISHED);
				} ?>
				...
				<th class="list-title" scope="row">
					<?php if (in_array($article->access, $this->user->getAuthorisedViewLevels())) : ?>
						<a class="p-2 d-block" href="<?php echo $link; ?>">
					...
					<?php else : ?>
						...
						?>
						<a href="<?php echo $link; ?>" class="register p-3">
					...
					<?php if (JVERSION < 4) {
						...
						if ($article->state == ContentComponent::CONDITION_UNPUBLISHED) : ?>
						<div>
