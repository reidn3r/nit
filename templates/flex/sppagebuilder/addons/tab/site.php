...
class SppagebuilderAddonTab extends SppagebuilderAddons {
	...
	public function render() {
		...
		$fluid_tab_width = (isset($settings->fluid_tab_width) && $settings->fluid_tab_width) ? ' '. $settings->fluid_tab_width : 'fluid';
		
		//Flex Style only
		//if($style == 'flex' || $settings->class == 'flex') { 
		if($style == 'flex') { 
			$flex_style = ' flex';
			$flex_css = ' d-flex';
			$flex_fill = 'flex-fill ';
		...
		//Tab Title
		$output .='<ul class="sppb-nav'. $flex_css .' sppb-nav-' . 
			$style 
			//. $fluid_tab_width 
			.'" role="tablist">';
		foreach ($settings->sp_tab_item as $key => $tab) {
            ...
            $output .='<li class="'. $flex_fill . ( ($key==0) ? "active" : "").'">';
	...
	public static function getTemplate(){
		...
		<style type="text/css">
			...
			#sppb-addon-{{ data.id }} .sppb-nav {list-style:none!important;}
		...
		<div class="sppb-addon sppb-addon-tab {{ data.class }}">
			...
			let fluid_tab_css = (data.fluid_tab_width == \'fluid\') ? \' d-flex\' : \'\';
			let fluid_tab_fill = (data.fluid_tab_width == \'fluid\') ? \'flex-fill \' : \'\';
            #>
            <div class="sppb-addon-content sppb-tab sppb-{{data.style}}-tab sppb-tab-{{navPosition}}">
                <ul class="sppb-nav{{ fluid_tab_css }} sppb-nav-{{ data.style }}">
                    <#
                    ...
                    #>
                        <li class="{{ fluid_tab_fill }}{{ active }}">
