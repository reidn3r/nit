...
function ckMobilemenuUpdateStyle(nothing, id, name) {
		...
		$html .= '<div class="btn btn-outline-secondary" onclick="CKBox.open({url: \'index.php?option=com_mobilemenuck&view=styles&tmpl=component&modal=1&returnFunc=ckMobilemenuSelectStyle\'})"><i class="fas fa-mouse-pointer "></i> ' . \Joomla\CMS\Language\Text::_('PLG_MOBILEMENUCK_SELECT') . '</div>';
		$html .= '<div class="btn btn-outline-secondary" onclick="if (jQuery(\'#' . $this->id . '\').val()) {CKBox.open({url: \'index.php?option=com_mobilemenuck&view=style&tmpl=component&modal=1&id=\'+jQuery(\'#' . $this->id . '\').val()+\'\'}) } else { alert(\'' . \Joomla\CMS\Language\Text::_('PLG_MOBILEMENUCK_SELECT_FIRST', true) . '\');}"><i class="fas fa-edit"></i> ' . \Joomla\CMS\Language\Text::_('PLG_MOBILEMENUCK_EDIT', true) . '</div>';
		$html .= '<div class="btn btn-outline-secondary" onclick="jQuery(\'#' . $this->id . '\').val(\'\');jQuery(\'#' . $this->id . 'name\').val(\'\');"><i class="fas fa-times"></i> ' . \Joomla\CMS\Language\Text::_('PLG_MOBILEMENUCK_CLEAR', true) . '</div>';
		$html .= '</div>';
