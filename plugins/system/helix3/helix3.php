...
class  plgSystemHelix3 extends CMSPlugin
{
    ...
    function onAfterRender() {
		...
		if ($params->get('strip_whitespace')) {
			...
			$sws_buffer = Factory::getApplication()->getBody();
		...
		//$sws_buffer = preg_replace('/([\s])\1+/', '', $sws_buffer); /* Removed (from Flex 3.9.9) double space: >< causing issues with javascript code */
		...
		if ($sws_buffer === null) exit("PCRE Error! File too big.\n");
			...
			{
				Factory::getApplication()->setBody($sws_buffer);
			}
			*/
			Factory::getApplication()->setBody($sws_buffer);
			return $sws_buffer;	
